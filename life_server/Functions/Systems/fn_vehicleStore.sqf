...
if(_impound) then
{
	...
	{
		_query = format["UPDATE vehicles SET active='0', four='1', reboot='0', fuel='%3', inventory='""[]""', gear='""[]""' WHERE pid='%1' AND plate='%2'",_uid,_plate, fuel _vehicle];
	...
	else
{
	...
	_query = format["UPDATE vehicles SET active='0', reboot='0', fuel='%3', inventory='""[]""', gear='""[]""' WHERE pid='%1' AND plate='%2'",_uid,_plate, fuel _vehicle];
	waitUntil {!DB_Async_Active};
	_thread = [_query,1] call DB_fnc_asyncCall;
	//waitUntil {scriptDone _thread};
	if(!isNil "_vehicle" && {!isNull _vehicle}) then {
