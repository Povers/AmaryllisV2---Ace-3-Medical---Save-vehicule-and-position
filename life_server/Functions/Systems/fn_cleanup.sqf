...
while {true} do
{
	...
	sleep (60 * 600);
	{
		...
		if(_vehicleClass in ["Car","Air","Ship","Armored","Submarine"]) then
		{
			...
			_units = {(_x distance _veh < 150000)} count playableUnits;
			...
			if(isNull _veh) then
			{
				...
				if(count _dbInfo > 0) then
				{
                    ...
                    _query = format["UPDATE vehicles SET active='0', reboot='0', fuel='%3' WHERE pid='%1' AND plate='%2'",_uid,_plate,fuel _veh];
