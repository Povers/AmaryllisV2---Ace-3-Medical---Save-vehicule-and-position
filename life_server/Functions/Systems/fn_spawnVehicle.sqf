...
_query = format["SELECT id, side, classname, type, pid, alive, active, plate, color, assure, fuel FROM vehicles WHERE id='%1' AND pid='%2'",_vid,_pid];

waitUntil{sleep (random 0.3); !DB_Async_Active};
_tickTime = diag_tickTime;
_queryResult = [_query,2] call DB_fnc_asyncCall;
_fuel = parseNumber(_queryResult select 10);
/*
diag_log "------------- Client Query Request spawn Vehicle 1-------------";
...
_query = format["UPDATE vehicles SET active='1', four='0', reboot='0' WHERE pid='%1' AND id='%2'",_pid,_vid];

waitUntil {!DB_Async_Active};
[_query,false] spawn DB_fnc_asyncCall;
/*
diag_log "------------- Client Query Request spawn Vehicle 2-------------";
diag_log format["QUERY: %1",_query];
diag_log format["Result: %1",_queryResult];
diag_log "------------------------------------------------";*/
if(typeName _sp == "STRING") then {
...
//diag_log format["spawn vehicle : %1",_vehicle];
