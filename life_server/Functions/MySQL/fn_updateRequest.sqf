...
switch (_side) do {
	case west: {
	// ADAPTATION WEST
		_civPosition = [_this select 7] call DB_fnc_mresArray; // 8?
		_alive = [_this select 8] call DB_fnc_bool;
		_query = format["UPDATE players SET name='%1', cash='%2', bankacc='%3', cop_gear='%4', cop_licenses='%5', civPosition='%7', alive='%8' WHERE playerid='%6'",_name,_cash,_bank,_gear,_licenses,_uid,_civPosition,_alive];
	};
	case civilian: {
	...
	case independent: { // PAS ENCORE TERMINE
		_civPosition = [_this select 7] call DB_fnc_mresArray;
		_alive = [_this select 8] call DB_fnc_bool;
		_query = format["UPDATE players SET name='%1', cash='%2', bankacc='%3', med_licenses='%4', med_gear='%6', civPosition='%7', alive='%8' WHERE playerid='%5'",_name,_cash,_bank,_licenses,_uid,_gear,_civPosition,_alive];
	};
};
