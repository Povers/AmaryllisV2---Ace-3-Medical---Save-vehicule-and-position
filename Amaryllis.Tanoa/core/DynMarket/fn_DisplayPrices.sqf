...
if (_cost<_costOld) then {
	...
	_arrowText = format ["<br/><br/><t color='#b01114'>↓ %1%2",_percent,"%"];
} else {
	if (_costOld<_cost) then {
		_percent = (100-((_costOld/_cost)*100));
		_arrowText = format ["<br/><br/><t color='#8ad672'>↑ %1%2",_percent,"%"];
	} else {
		_percent = (100-((_costOld/_cost)*100));
		_arrowText = format [""];
	};
};
_cashdisplayold ctrlSetStructuredText parseText format ["%1€",floor(_costOld)];