...
switch (true) do
{
	case (_vehicle isKindOF "LandVehicle"):
	{
		...
		{
			[[_vehicle,(Fuel _vehicle) + 0.20],"life_fnc_setFuel",_vehicle,false] spawn life_fnc_MP;
		}
			else
		{
			_vehicle setFuel ((Fuel _vehicle) + 0.20);
		};
	};
	...
	case (_vehicle isKindOf "Air"):
	{
		if(!local _vehicle) then
		{
			[[_vehicle,(Fuel _vehicle) + 0.10],"life_fnc_setFuel",_vehicle,false] spawn life_fnc_MP;
		}
			else
		{
