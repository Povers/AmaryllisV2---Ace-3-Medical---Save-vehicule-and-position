...
*/
private["_building","_door","_doors","_cpRate","_title","_progressBar","_titleText","_cp","_ui","_uid","owner","_active","_bank"];
_active = 1;
_bank = 0;
_building = [_this,0,ObjNull,[ObjNull]] call BIS_fnc_param;
if(isNull _building) exitWith {};
if(!(_building isKindOf "House_F")) exitWith {hint "Vous n'êtes pas proche d'une porte."};
...
if((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _building OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _building) then {
	...
	if(!([_uid] call life_fnc_isUIDActive)) then {_active = 0;};
};

if(({side _x == west} count playableUnits < 8) && _bank == 1) exitwith {hint "Il n'y a pas suffisamment de membre du C.O.S !"};
if (_active == 0) exitWith {hint "Le propriétaire est endormi ..."};
...
if (_bank == 1) then {
	...
	//[[_building, "AlarmHouse",200],"life_fnc_playSound",true,false] spawn life_fnc_MP;
};
