...
if((profileName != _this select 1) && (__GETC__(life_adminlevel) < 1))exitWith {
hint format["Votre personnage sur le serveur se nomme %1",_this select 1];
["NewName",false,false] call BIS_fnc_endMission;
};
...
switch(playerSide) do {
	case west: {
		...
		civ_position = _this select 10; // 10 par defaut
	...
	case civilian: {
		civ_position = _this select 9; // 9 par defaut
