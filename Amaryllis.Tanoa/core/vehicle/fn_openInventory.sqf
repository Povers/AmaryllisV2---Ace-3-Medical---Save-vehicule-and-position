...
*/
private["_vehicle","_veh_data","_vehtrunk"];
if(dialog) exitWith {};
if((time - life_action_delay) < 0.5) exitWith {hint "Vous ne pouvez pas effectuer cette action aussi vite !"}; // TODO : TEST
...
if (player distance _vehicle > 8) exitWith {hint "Il est loin le coffre non ?"};
...
[_vehicle] call life_fnc_vehStoreInventory;
if(_vehicle isKindOf "House_F") then {
	private["_mWeight"];
	_mWeight = 0;
...
// diag_log format ["------------------------------------------------------------------------------------>contenance : %1",_veh_data];
...
_vehicle spawn
{
	...
	if(_this isKindOf "House_F") then {
		...
		[[_this],"TON_fnc_updateVehTrunk",false,false] spawn life_fnc_MP;
		[3] call SOCK_fnc_updatePartial;
	};
};