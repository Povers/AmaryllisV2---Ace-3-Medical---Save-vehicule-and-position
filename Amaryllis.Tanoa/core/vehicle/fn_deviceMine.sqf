...
_resourceZones = ["apple_1","grape","iron","gold","corundum","rye","wood","wheat","hop","part_of_wreck","sulphide","blacklead_ore","clay","tobacco","diamond"];
_zone = "";
...
_item = switch(true) do {
	...
	case (_zone in ["grape"]): {"grape"};
	case (_zone in ["iron"]): {"iron_ore"};
	case (_zone in ["gold"]): {"gold"};
	case (_zone in ["corundum"]): {"corundum"};
	case (_zone in ["rye"]): {"rye"};
	case (_zone in ["wood"]): {"wood"};
	case (_zone in ["wheat"]): {"wheat"};
	case (_zone in ["hop"]): {"hop"};
	case (_zone in ["part_of_wreck"]): {"part_of_wreck"};
	case (_zone in ["epaves"]): {"epaves"};
	case (_zone in ["blacklead_ore"]): {"blacklead_ore"};
	case (_zone in ["silver"]): {"silver"};
	case (_zone in ["clay"]): {"clay"};
	case (_zone in ["tobacco"]): {"tobacco"};
	case (_zone in ["diamond"]): {"rough_diamond"};
	default {""};
};
