...
switch (_mode) do
{
	...
	case 3:
	{
		ama_sound_foot = round(_value);
		ctrlSetText[2931,format["%1",round(_value)]];
		[] call life_fnc_updateSound;
	};
	
	case 4:
	{
		ama_sound_car = round(_value);
		ctrlSetText[2933,format["%1",round(_value)]];
		[] call life_fnc_updateSound;
	};
	
	case 5:
	{
		ama_sound_air = round(_value);
		ctrlSetText[2935,format["%1",round(_value)]];
		[] call life_fnc_updateSound;
	};
	
	case 6:
	{
		ama_sound_boat = round(_value);
		ctrlSetText[2937,format["%1",round(_value)]];
		[] call life_fnc_updateSound;
	};
};