...
{
	if(!createDialog "SettingsMenu") exitWith {hint "Quelque chose ne va pas ..."};
};
...
ctrlSetText[2931, format["%1", ama_sound_foot]];
ctrlSetText[2933, format["%1", ama_sound_car]];
ctrlSetText[2935, format["%1", ama_sound_air]];

//Setup Sliders range
{ slidersetRange [_x,100,12000];} foreach [2901,2911,2921];
{ slidersetRange [_x,1,100];} foreach [2930,2932,2934];
//Setup Sliders speed
{ ((findDisplay 2900) displayCtrl _x) sliderSetSpeed [100,100,100]; } foreach [2901,2911,2921];
{ ((findDisplay 2900) displayCtrl _x) sliderSetSpeed [1,1,1]; } foreach [2930,2932,2934];
//Setup Sliders position
{
	sliderSetPosition[_x select 0, _x select 1];
} foreach [[2901,tawvd_foot],[2911,tawvd_car],[2921,tawvd_air],[2930,ama_sound_foot],[2932,ama_sound_car],[2934,ama_sound_air]];
