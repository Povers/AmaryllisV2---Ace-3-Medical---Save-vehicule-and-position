...
//Near players
_near_units = [];
{ if(player distance _x < 100000) then {_near_units pushBack _x};} foreach playableUnits;
{
	if(!isNull _x && alive _x && player distance _x < 50000 && _x != player) then
	{
		if (cursortarget == _x) then {
			...
			lbSortByValue _near_i;
		};
		_near lbAdd format["%1",_x getVariable["realname",name _x], side _x];
		_near lbSetData [(lbSize _near)-1,str(_x)];
		_near_i lbAdd format["%1",_x getVariable["realname",name _x], side _x];
...
ctrlSetText[2009,format["%1 / %2", life_carryWeight, life_maxWeight]];
{
	...
	if(_val > 0) then
	{
		_inv lbAdd format["%1  %2",_val,_str];
