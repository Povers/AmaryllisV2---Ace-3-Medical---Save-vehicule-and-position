...
switch (life_veh_shop select 0) do
{
	...
	case "civ_depanneur": {_ret = license_civ_depannage;};
	case "gouv_car": {_ret = license_civ_gouv;};
	case "civ_ship": {_ret = license_civ_boat;};
	case "civ_air": {_ret = license_civ_pilotciv;};
	case "cop_air": {_ret = true;};
	case "cop_airhq": {_ret = true;};
	case "civ_truck": {_ret = license_civ_truck;};
	case "reb_car": {_ret = license_civ_criminel;};
