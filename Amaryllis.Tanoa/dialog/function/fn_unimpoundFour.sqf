...
if(lbCurSel 2852 == -1) exitWith {hint localize "STR_Global_NoSelection"};
_vehicle = lbData[2852,(lbCurSel 2852)];
_vehicle = (call compile format["%1",_vehicle]) select 0;
_vid = lbValue[2852,(lbCurSel 2852)];
...
if(_price == -1) then {_price = 1000000;} else {_price = (__GETC__(life_garage_prices) select _price) select 1;};
